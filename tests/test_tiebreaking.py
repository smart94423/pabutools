...
class TestTieBreaking(TestCase):
    ...
    def test_tie_breaking_rules(self):
        ...
        assert app_score_tie_breaking.order(instance, profile, p)[:3] == [p[2], p[3], p[4]]
        assert app_score_tie_breaking.untie(instance, profile, p[3:]) == p[3]
        assert app_score_tie_breaking.order(instance, profile, p[3:])[:2] == [p[3], p[4]]

        assert min_cost_tie_breaking.untie(instance, profile, p) in (p[0], p[3])
        assert min_cost_tie_breaking.order(instance, profile, p) in ([p[0], p[3], p[2], p[1], p[5], p[4]],
                                                                     [p[3], p[0], p[2], p[1], p[5], p[4]])
        assert min_cost_tie_breaking.untie(instance, profile, p[3:]) == p[3]
        assert min_cost_tie_breaking.order(instance, profile, p[3:]) == [p[3], p[5], p[4]]

        assert max_cost_tie_breaking.untie(instance, profile, p) == p[4]
        assert max_cost_tie_breaking.order(instance, profile, p) in ([p[4], p[5], p[1], p[2], p[3], p[0]],
                                                                     [p[4], p[5], p[1], p[2], p[0], p[3]])
        assert max_cost_tie_breaking.untie(instance, profile, p[3:]) == p[4]
        assert max_cost_tie_breaking.order(instance, profile, p[3:]) == [p[4], p[5], p[3]]
