...
def profile_sat_map(prof):
    if isinstance(prof, AbstractApprovalProfile):
        return [CC_Sat, Cost_Sqrt_Sat, Cost_Log_Sat, Cardinality_Sat, Relative_Cardinality_Sat, Cost_Sat,
                Relative_Cost_Sat, Relative_Cost_Approx_Normaliser_Sat, Effort_Sat]
    if isinstance(prof, AbstractCardinalProfile):
        return [CC_Sat, Cardinality_Sat, Relative_Cardinality_Sat, Cost_Sat,
                Relative_Cost_Sat, Relative_Cost_Approx_Normaliser_Sat, Effort_Sat, Additive_Cardinal_Sat]
    if isinstance(prof, AbstractOrdinalProfile):
        return [Cardinality_Sat, Relative_Cardinality_Sat, Cost_Sat,
                Relative_Cost_Sat, Relative_Cost_Approx_Normaliser_Sat, Effort_Sat, Additive_Borda_Sat]
            ...
            for rule in ALL_SAT_RULES:
                for profile in [prof, prof.as_multiprofile()]:
                    for sat_class in profile_sat_map(profile):
                        ...
                        rule(instance, profile, sat_profile=profile.as_sat_profile(sat_class))
