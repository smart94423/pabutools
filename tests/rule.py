...
def test_greedy_welfare_approval():
    ...
    assert greedy_welfare_approval(instance, profile, cost_sat(instance, profile)) == ["p1", "p3"]
    assert greedy_welfare_approval(instance, profile, cardinality_sat(instance, profile)) == ["p1", "p4"]
    irresolute_out = [["p1", "p3"], ["p1", "p4"], ["p2"], ["p3", "p4"]]
    assert greedy_welfare_approval(instance, profile, cost_sat(instance, profile), resoluteness=False) == irresolute_out
