...
class TestBallot(TestCase):
    ...
    def test_approval_ballot(self):
        ...
        p5 = Project("p5", 2)
        p6 = Project("p6", 2)
        ballot = ApprovalBallot([p1, p2, p3, p4], name="AppBallot", meta={"metakey": "value"})
        assert p1 in ballot
        assert p2 in ballot
        assert p3 in ballot
        assert p4 in ballot
    ...
    def test_cardinal_ballot(self):
        ...
        ballot = CardinalBallot({projects[1]: 100, projects[2]: 74, projects[3]: 12, projects[4]: 7, projects[5]: -41},
                                name="CardBallot", meta={"MetaKey": "Value"})
        ...
        frozen_ballot1 = FrozenCardinalBallot({projects[1]: 100, projects[2]: 74, projects[3]: 12, projects[4]: 7,
                                               projects[5]: -41})
        frozen_ballot2 = ballot.frozen()
        frozen_ballot3 = FrozenCardinalBallot(ballot)
        for p in ballot:
            assert ballot[p] == frozen_ballot1[p] == frozen_ballot2[p] == frozen_ballot3[p]
        assert ballot.name == frozen_ballot2.name == frozen_ballot3.name
        assert ballot.meta == frozen_ballot2.meta == frozen_ballot3.meta
    ...
    def test_ordinal_ballot(self):
        ...
        assert list(ballot) == list(frozen_ballot1) == list(frozen_ballot2) == list(frozen_ballot3)
        assert ballot.name == frozen_ballot2.name == frozen_ballot3.name
        assert ballot.meta == frozen_ballot2.meta == frozen_ballot3.meta
